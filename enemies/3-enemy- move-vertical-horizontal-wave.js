/** @type {HTMLCanvasElement} */

const canvas = document.getElementById("canvas1");
const ctx = canvas.getContext("2d");
const CANVAS_WIDTH = (canvas.width = 500);
const CANVAS_HEIGHT = (canvas.height = 700);
const numberOfEnemies = 500;
let enemiesArray = [];

let gameFrame = 0;

// Factoty function
class Enemy {
  constructor() {
    this.image = new Image();
    this.image.src = "./images/enemy3.png";
    this.speed = Math.random() * 4 + 1; // 1 ... 5
    this.spriteWidth = 218;
    this.spriteHeight = 177;
    this.width = this.spriteWidth / 2;
    this.height = this.spriteHeight / 2;
    this.x = Math.random() * (canvas.width - this.width);
    this.y = Math.random() * (canvas.height - this.height);
    this.frame = 0;
    this.flapSpeed = Math.floor(Math.random() * 3 + 1);
    this.angle = Math.random() * 100;
    this.angleSpeed = Math.random() * 0.5 + 0.5; // 0 ... 2
    // this.curve = Math.random() * 200 + 50;
  }
  update() {
    // move animation
    this.x =
      (canvas.width / 2) * Math.sin((this.angle * Math.PI) / 90) +
      (canvas.width / 2 - this.width / 2); // -250 ... 250
    this.y =
      (canvas.height / 2) * Math.cos((this.angle * Math.PI) / 70) +
      (canvas.height / 2 - this.height / 2); // -350 ... 350
    this.angle += this.angleSpeed;
    if (this.x + this.width < 0) this.x = canvas.width; // left to rigth

    // animate sprites
    if (gameFrame % this.flapSpeed === 0) {
      this.frame > 4 ? (this.frame = 0) : this.frame++;
    }
  }
  draw() {
    // ctx.strokeRect(this.x, this.y, this.width, this.height);
    ctx.drawImage(
      this.image,
      this.frame * this.spriteWidth,
      0,
      this.spriteWidth,
      this.spriteHeight, // to "cut" in image
      this.x,
      this.y,
      this.width,
      this.height // to "paste" on canvas
    );
  }
}

// Create enemies objts
for (let i = 0; i < numberOfEnemies; i++) {
  enemiesArray.push(new Enemy());
}
console.log(enemiesArray);

function animate() {
  ctx.clearRect(0, 0, CANVAS_WIDTH, CANVAS_HEIGHT);

  enemiesArray.forEach((enemy) => {
    enemy.update();
    enemy.draw();
  });
  gameFrame++;
  requestAnimationFrame(animate);
}
animate();
